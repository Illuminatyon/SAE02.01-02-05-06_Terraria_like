...
public abstract class newAbstractInventory {
    //protected final ArrayList<InventorySlot> slots;
    protected InventorySlot[][] slotsArray;
    protected int size;
    protected int slotsOccupied;
    protected int rowsNumber;

    protected newAbstractInventory(int rows) {
        this.size = rows*10;
        this.rowsNumber = rows;
        this.slotsOccupied = 0;
        //this.slots = new ArrayList<>(size);
        this.slotsArray = new InventorySlot[rows][10];
        initializeSlots();
    }
...
//        }
        ...
        for (int i = 0; i < rowsNumber; i++) {
            for (int j = 0; j < 10; j++) {
                slotsArray[i][j] = new InventorySlot(j+i*10);
            }
        }
    }
